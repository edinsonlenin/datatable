...
function ValidatorFocus()
{
    ...
    for (i = 0; i < Page_Validators.length; i++) {
        if (!Page_Validators[i].isvalid) {
			...
			if((Page_Validators[i].controltovalidate).indexOf('wdt')>-1)
				document.getElementById(Page_Validators[i].controltovalidate + "_t").focus();
			else
				if((Page_Validators[i].controltovalidate).indexOf('txtControl')>-1){
					arrControlName = (Page_Validators[i].controltovalidate).split("_");
					document.getElementById(arrControlName[1]+ "_input").style.background="highlight";
				}
				else
