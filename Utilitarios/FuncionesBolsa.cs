...
namespace Seriva.Bolsa.Presentacion.Utilitarios
{
    ...
    public class FuncionesBolsa
	{
		...
		public static bool b_CargarCuentasMDC(DropDownList wcbCuentaMDC, HiddenField storageData, int codigoCliente, bool tieneCustodia) 
		{
            bool existeCuentas = false;
            try
            {
				wcbCuentaMDC.Items.Clear();
                ...
                {
                    wcbCuentaMDC.DataTextField = "NUMEROCUENTAMDC";
                    wcbCuentaMDC.DataValueField = "CODCUENTAMDC";
					wcbCuentaMDC.DataSource = cuentasMDC.Tables[0];
                    wcbCuentaMDC.DataBind();

					storageData.Value = DatatableHelper.SerializeDatatableForDrowpDownList(cuentasMDC.Tables[0], wcbCuentaMDC.DataValueField, wcbCuentaMDC.DataTextField);

                    if (wcbCuentaMDC.Items.Count > 0)
					{
		...
		public static bool b_CargarMancomunos(DropDownList wcbMancomunos, HiddenField storageData, int codigoCuentaMDC) 
		{
            bool existeMancomunos = false;
            try
            {
                wcbMancomunos.Items.Clear();
                ...
                {
                    FuncionesGenerales.of_LlenarWebComboVariasColumnas(wcbMancomunos, mancomunos.Tables[0], storageData, "CODCUENTACUSTODIA", "NUMERO", false);
                    if (wcbMancomunos.Items.Count > 0)
                    {
		...
		public static void CargarRepresentantes(int codigoCliente, DropDownList ddlRepresentante, string tipoCliente, string descripcion)
		{
			...
			if(representanteLegalDataSet.SER_REPRESENTANTECLIENTEBOLSA.Rows.Count > 0)
		    {
			   ...
			   //ListItem nuevoItemSeleccioneUnaOpacion = new ListItem(etiquetaSeleccioneUnaOpcion,ConstantesBolsa.SELECCION_VALOR_NULO);
			   ddlRepresentante.Items.Insert(0, new ListItem("SELECCIONE UNA OPCIÓN",ConstantesBolsa.SELECCION_VALOR_NULO));
			}
		...
		public static int ValidarOperacionesReporte(e_ValidacionReporte origenValidacion,out string mensaje)
		{
			...
			//JM - 23-07-15 Cambio: Mensaje de validación de hora, solo para ingreso de ordenes.
			//if (codigoValidacion == 2)
			if (codigoValidacion == 2 && origenValidacion == e_ValidacionReporte.IngresoOrdenesRdo)
