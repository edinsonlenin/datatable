...
class BaseWebCombo {
    ...
    #defaults = {
        ...
        resultsAdapter: $.fn.select2.amd.require('select2/data/customResultsAdapter'),
        dataAdapter: $.fn.select2.amd.require('select2/data/customAdapter'),
        triggerSelectOnInit: true
    }

    constructor(select2Id, options) {
        this.#select2Id = select2Id;
        this.#options = { ...this.#defaults, ...options };
    }

    init() {
        this.#select2 = $(this.#select2Id).select2(this.#options);

        if (this.#options.selectHandler instanceof Function) {
            $(this.#select2Id).on("select2:select", this.#options.selectHandler);
        }

        if (this.#options.changeHandler instanceof Function) {
            $(this.#select2Id).on("change.select2", this.#options.changeHandler);
        }

        if (this.#options.triggerSelectOnInit) {
            let select2Data = $(this.#select2Id).select2('data');

            if(select2Data && select2Data.length > 0 ) {
                let rawData = select2Data[0];

                delete rawData["element"];
                //delete rawData["_resultId"];
                // delete rawData["disabled"];
                // delete rawData["selected"];
                // delete rawData["title"];

                $(this.#select2Id).trigger({
                    type: 'select2:select',
                    params: {
                        data: rawData
                    }
    ...
    updateData(data) {
        this.#select2.data('select2').dataAdapter.updateOptions(data);
    }
}