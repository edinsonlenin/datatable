class BaseDataTable {
    ...
    get name() {
        return this.#_tableId.substring(1);
    }
    ...
    init() {
        ...
        if (this.options.moveTo)
            $(this.tableId).on('click', 'tbody tr', function (e) {
                ...
                let clickedRow = $(this);
                let rowData = that.datatableObject.row(clickedRow).data();
                moveRowHandler(that.name,that.options.moveTo,rowData);
            });
...
function moveRowHandler(origen, destino, data){
    let tablaOrigenBase = eval("tabla" + origen + "Base")
    let tablaDestinoBase = eval("tabla" + destino + "Base")

    let dsOrigen = tablaOrigenBase.options.data;
    let dsDestino = tablaDestinoBase.options.data;

    let tabla1 = tablaOrigenBase.datatableObject;
    let tabla2 = tablaDestinoBase.datatableObject;
    //var data = tabla1.row(fila).data();
    var index = dsOrigen.findIndex(x => x.id == data.id);
    dsOrigen.splice(index, 1);
    tabla1.clear();
    tabla1.rows.add(dsOrigen);
    tabla1.draw();

    dsDestino.push(data);
    tabla2.clear();
    tabla2.rows.add(dsDestino);
    tabla2.draw();

    let cantidadFilas = tabla2.rows().count();
    tabla2.row(cantidadFilas-1).scrollTo();
    $("#destino tr").eq(cantidadFilas).addClass('bk_yellow');
    //$(tabla2.row(cantidadFilas-1)).addClass('bk_yellow')
}
